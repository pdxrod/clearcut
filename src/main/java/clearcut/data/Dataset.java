...
public class Dataset {
	...
	public Dataset(String jdbcURL, String database, String loginName,
			String password) {
	...
	public void delete(String tableName, String whereClause)
		...
		Connection con = null;
		try {
			con = DriverManager.getConnection(this.url, this.login,
					this.password);
			new Delete(con, tableName, whereClause);
		} finally { // If you can't close it, it's null, isn't open, or you
	...
	public void update(String tableName, Map<String, String> values,
			String whereClause) throws DataException, SQLException {
		Connection con = null;
		try {
			con = DriverManager.getConnection(this.url, this.login,
					this.password);
	...
	Map<String, String> insertOrUpdateValues, String whereClause)
			throws DataException, SQLException {
		Connection con = null;
		try {
			con = DriverManager.getConnection(this.url, this.login,
	...
	public List<Map<String, String>> method(String procedureName,
			String[] inputs) throws DataException, SQLException {
		Connection con = null;
		try {
			con = DriverManager.getConnection(this.url, this.login,
					this.password);
	...
	public Select select(String tableName, String[] columns, String whereClause)
			throws DataException, SQLException {
		Connection con = null;
		try {
			con = DriverManager.getConnection(this.url, this.login,
	...
	public List<String[]> rows(String tableName, String[] columnNames,
			String whereClause) throws DataException, SQLException {
		return this.select(tableName, columnNames, whereClause).rows();
	}

	public List<Map<String, String>> table(String tableName,
			String[] columnNames, String whereClause) throws DataException,
			SQLException {
	...
	public static String where(String whereClause) {
		if (whereClause == null || whereClause.trim().equals(""))
			return " ";
		if (whereClause.toLowerCase().trim().indexOf("where") == 0)
			return " " + whereClause;
	...
	public static String quote(String quotable) {
		if (quotable.indexOf(QUOTE) < 0)
			return Quote + quotable + Quote;
		return Quote + quotable.replaceAll(Quote, DoubleQuote) + Quote; // MySQL
																		// doesn't
																		// care
																		// but
																		// M$$QL
