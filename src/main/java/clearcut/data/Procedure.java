...
public class Procedure extends Results {
		...
		throws DataException, SQLException      
  {
    ...
    if( inputs != null ) for( int i = 0; i < inputs.length; i ++ ) questionMarks += "?, ";

    if( questionMarks.endsWith( "?, " ) ) questionMarks = questionMarks.substring( 0, questionMarks.length() - 2 ); // Remove trailing comma    
    questionMarks += " )";
	...
	this.init( names );  
// The procedure might be update-only. In MySQL, the only way to avoid an exception is to look at the column count.
    if( columnCount > 0 ) while (resultset.next()) {
...
// Some drivers will complain if there is no result set - the only way to find out is to look at the message
